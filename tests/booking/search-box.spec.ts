...
test.describe('Search box — Destination dropdown', () => {
  ...
  test('opens when the Destination trigger is clicked', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-cruiseappy="search_destination"] .search-form-dropdown'),
    ).toBeVisible();
  });
  ...
  test('contains a search text input inside the dropdown', async ({ page }) => {
    await page.locator('[data-cruiseappy="search_destination"]').click();
    await expect(
      page.locator('[data-cruiseappy="search_destination"] input.search'),
    ).toBeVisible();
  });
...
test.describe('Search box — Cruise Line dropdown', () => {
  ...
  test('opens when the Cruise Line trigger is clicked', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-cruiseappy="search_cruise_line"] .search-form-dropdown'),
    ).toBeVisible();
  });
  ...
  test('contains a search text input inside the dropdown', async ({ page }) => {
    await page.locator('[data-cruiseappy="search_cruise_line"]').click();
    await expect(
      page.locator('[data-cruiseappy="search_cruise_line"] input.search'),
    ).toBeVisible();
  });
...
test.describe('Search box — Departure Date calendar', () => {
  ...
  test('opens when the Departure Date trigger is clicked', async ({ page }) => {
    await page.locator('[data-cruiseappy="search_dates"]').click();
    await expect(page.locator('.calendar-monthpicker')).toBeVisible();
  });

  test('shows year headings for at least two future years', async ({ page }) => {
    await page.locator('[data-cruiseappy="search_dates"]').click();
    const calendar = page.locator('.calendar-monthpicker');
    await expect(calendar).toBeVisible();
    const groupCount = await calendar.locator('.year-group').count();
  ...
  test('shows selectable (non-disabled) months for upcoming months', async ({ page }) => {
    ...
    const enabledLabel = page.locator(`label[for="sf_date_${month2}"]`);
  ...
  test('selecting a single month updates the trigger label', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-cruiseappy="search_dates"]'),
    ).toContainText(formatMonthLabel(month1));
  });
  ...
  test('selecting two months shows a date range in the trigger', async ({ page }) => {
    ...
    const trigger = page.locator('[data-cruiseappy="search_dates"]');
  ...
  test('search button href contains startdate and enddate params after month selection', async ({ page }) => {
    ...
    await expect(page.locator('[data-cruiseappy="search"]')).toHaveAttribute('href', /startdate=/, { timeout: 10_000 });
    await expect(page.locator('[data-cruiseappy="search"]')).toHaveAttribute('href', /enddate=/);
  });
  ...
  test('past months are rendered as disabled and cannot be selected', async ({ page }) => {
    ...
    const pastLabel = page.locator('label[for="sf_date_2026-01-01"]');
...
test.describe('Search box — Departure Port dropdown', () => {
  ...
  test('opens when the Departure Port trigger is clicked', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-cruiseappy="search_port"] .search-form-dropdown'),
    ).toBeVisible();
  });
  ...
  test('contains a search text input inside the dropdown', async ({ page }) => {
    await page.locator('[data-cruiseappy="search_port"]').click();
    await expect(
      page.locator('[data-cruiseappy="search_port"] input.search'),
    ).toBeVisible();
  });
...
test.describe('Search box — Search button', () => {
  ...
  test('shows "Search" by default when no filters are selected', async ({ page }) => {
    ...
    const btnText = (await page.locator('[data-cruiseappy="search"]').innerText()).trim();
    expect(btnText).toMatch(/^search$/i);
  });

  test('href points to the /search/ path by default', async ({ page }) => {
    const href = await page.locator('[data-cruiseappy="search"]').getAttribute('href');
  ...
  test('results page loads with a cruise count after clicking Search', async ({ page }) => {
    ...
    const countText = await page.locator('.total-results').innerText();
